use quarter::{LoopStack, parse_tokens, Dictionary, Stack};
...
fn test_if_then_true() {
    ...
    dict.execute_word("TEST", &mut stack, &mut loop_stack).unwrap();
...
fn test_if_with_stack_operations() {
    ...
    stack.push(21);
    dict.execute_word("DOUBLE_IF_POSITIVE", &mut stack, &mut loop_stack)
        .unwrap();
    assert_eq!(stack.pop(), Some(42));
