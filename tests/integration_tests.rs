...
use quarter::{execute_line, load_file, Dictionary, LoopStack, ReturnStack, Stack, Memory, RuntimeContext, CompilerConfig, ExecutionOptions};
use std::fs;
use std::io::Write;
...
fn test_execute_line_simple_expression() {
...

    {
        ...
        execute_line("5 3 +", &mut ctx, config, options, &mut HashSet::new()).unwrap();
    }
...
fn test_execute_line_word_definition() {
    ...
    let config = CompilerConfig::new(true, false, false);
    let options = ExecutionOptions::new(false, false);

    {
        ...
        execute_line(": SQUARE DUP * ;", &mut ctx, config, options, &mut HashSet::new()).unwrap();

        // Use SQUARE
        execute_line("5 SQUARE", &mut ctx, config, options, &mut HashSet::new()).unwrap();
    }
...
fn test_execute_line_if_then_error() {
    ...
    let result = execute_line("1 IF 42 THEN", &mut ctx, config, options, &mut HashSet::new());
...
fn test_execute_line_empty() {
    ...
    let result = execute_line("", &mut ctx, config, options, &mut HashSet::new());
    assert!(result.is_ok());
}
...
fn test_execute_line_invalid_word() {
    ...
    let result = execute_line("NONEXISTENT", &mut ctx, config, options, &mut HashSet::new());
    assert!(result.is_err());
}
...
fn test_execute_line_incomplete_definition() {
    ...
    // Missing semicolon
    let result = execute_line(": SQUARE DUP *", &mut ctx, config, options, &mut HashSet::new());
    assert!(result.is_err());
...
fn test_load_file_simple() {
...

    {
        ...
        load_file(test_file, &mut ctx, config, options, &mut HashSet::new()).unwrap();
    }
...
fn test_load_file_nonexistent() {
    ...
    let result = load_file("/tmp/nonexistent_file.qtr", &mut ctx, config, options, &mut HashSet::new());
    assert!(result.is_err());
...
fn test_include_simple() {
...

    {
        ...
        execute_line(&format!("INCLUDE {}", lib_file), &mut ctx, config, options, &mut HashSet::new()).unwrap();
    }
...

    {
        ...
        execute_line("5 DOUBLE", &mut ctx, config, options, &mut HashSet::new()).unwrap();
    }
...
fn test_include_nested() {
...

    {
        ...
        execute_line(&format!("INCLUDE {}", lib2_file), &mut ctx, config, options, &mut HashSet::new()).unwrap();

        // WORD1 from lib1 should be defined
        execute_line("WORD1", &mut ctx, config, options, &mut HashSet::new()).unwrap();
    }
...

    {
        ...
        execute_line("WORD2", &mut ctx, config, options, &mut HashSet::new()).unwrap();
    }
...
fn test_include_nonexistent() {
    ...
    let result = execute_line("INCLUDE /tmp/nonexistent_lib.qtr", &mut ctx, config, options, &mut HashSet::new());
...
fn test_include_missing_filename() {
    ...
    let result = execute_line("INCLUDE", &mut ctx, config, options, &mut HashSet::new());
    assert!(result.is_err());
